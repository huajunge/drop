...
import com.cog.DropInn.ReservationDetail;
...
public class MyReservationAdapter extends BaseAdapter {
	...
	public MyReservationAdapter(Activity activity, List<Movie> movieItems) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null)
			convertView = inflater.inflate(R.layout.reservationlist,parent,false);
    ...
    System.out.println("roomid in adapter search activity"+m.getid());
		...
		// thumbnail image
		thumbNail1.setImageUrl(m.getThumbnailUrl(), imageLoader);
		thumbNail1.setScaleType(ScaleType.FIT_XY);
				...
				country.setText(addr);
				  ...
				  thumbNail.setOnClickListener(new View.OnClickListener() {
			            ...
			            public void onClick(View view) {
			            	...
			            	Intent search=new Intent(activity,ReservationDetail.class);
			            	search.putExtra("room_id", m.getid());
			            	search.putExtra("userid",userid);
			            	search.putExtra("address", m.getaddress());
			            	search.putExtra("image", m.getThumbnailUrl1());
			            	search.putExtra("title", m.getTitle());
			            	search.putExtra("price", india2);
			            	search.putExtra("room_type", m.getid1());
			            	search.putExtra("profimage", m.getThumbnailUrl());
			             	search.putExtra("symbol", m.getsymbol());
			            	search.putExtra("checkin", m.getcheckin());
			            	search.putExtra("checkout", m.getcheckout());
			            	search.putExtra("guest", m.guest());
			            
			            	search.putExtra("status", m.tripstatus());
			              	search.putExtra("symbol1", m.getsymbol());
